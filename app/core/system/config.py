...
def parse_dict_of_lists_from_env(prefix, default_dict=None):
    ...
    # we will look for all the env vars with the given prefix.
    for key,value in os.environ.items():
        if key.startswith(prefix):
            endpoint = key[len(prefix):].lower()   # extracting the endpoint name
...
class Settings:
    ...
    Loads from the environment variables and applies defaults.
    """
    ...
    def __init__(self) -> None:
        ...
        # Application Basics
        self.PROJECT_NAME = os.getenv("PROJECT_NAME", "Agents")
        self.VERSION = os.getenv("VERSION", "1.0.0")
        self.API_V1_STR = os.getenv("API_V1_STR", "/api/v1")
        self.DEBUG = os.getenv("DEBUG", "false").lower() in ("true", "1", "t", "yes")

        # Parse CORS origins using the helper functions
        ...
        self.LONG_TERM_MEMORY_EMBEDDING_MODEL = os.getenv("LONG_TERM_MEMORY_EMBEDDING_MODEL", "text-embedding-3-small")
